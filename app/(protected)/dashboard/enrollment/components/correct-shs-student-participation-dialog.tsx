      ...
      <ScrollArea className="min-h-0 flex-1"><div className="space-y-5 px-4 py-5 sm:px-6">
        ...
        {contextQuery.data ? <>
          ...
          {preview ? <>
            ...
            <div className="rounded-lg border bg-muted/20 p-4 text-sm"><p className="font-medium">Authoritative scope</p><p className="mt-1 text-muted-foreground">{getShsSubjectClassificationLabel(preview.source.kind)} | selected {preview.source.selectedTerm.name} | replacement Terms: {preview.source.plannedTerms.map(({ name }) => name).join(", ")}</p><p className="mt-1 text-muted-foreground">Results: {preview.source.resultStates.map(({ name, status }) => `${name}: ${status ?? "none"}`).join("; ")}</p>{preview.policy ? <p className="mt-1 text-muted-foreground">Elective policy: {preview.policy.minimumElectives}-{preview.policy.maximumElectives}; active count: {preview.policy.activeElectiveCount}.</p> : null}</div>
