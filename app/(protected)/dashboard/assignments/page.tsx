    ...
    <div className="space-y-6 p-6">
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="mb-4 flex flex-wrap items-end gap-2"><SegmentedNavigation ariaLabel="Teaching assignment view"><button type="button" aria-pressed={view === "matrix"} className={cn(buttonVariants({ variant: view === "matrix" ? "secondary" : "ghost", size: "sm" }))} onClick={() => setView("matrix")}>Teaching Matrix</button><button type="button" aria-pressed={view === "history"} className={cn(buttonVariants({ variant: view === "history" ? "secondary" : "ghost", size: "sm" }))} onClick={() => setView("history")}>History</button></SegmentedNavigation>{view === "matrix" && <><span className="ml-2 pb-1 text-sm text-muted-foreground">{matrixQuery.data?.academicYear.label ?? "Active Academic Year"}</span><div className="space-y-1"><label className="text-sm font-medium" htmlFor="assignment-grade">Grade</label><Select value={gradeLevel} onValueChange={(value) => { setGradeLevel(value as typeof gradeLevel); setTermId(null); }}><SelectTrigger id="assignment-grade" aria-label="Grade"><SelectValue /></SelectTrigger><SelectContent>{["7", "8", "9", "10", "11", "12"].map((grade) => <SelectItem key={grade} value={grade}>Grade {grade}</SelectItem>)}</SelectContent></Select></div><div className="space-y-1"><label className="text-sm font-medium" htmlFor="assignment-term">Term focus</label><Select value={termId ?? "ALL"} onValueChange={(value) => setTermId(value === "ALL" ? null : value ?? null)}><SelectTrigger id="assignment-term"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="ALL">All Terms</SelectItem>{matrixQuery.data?.terms.map((term) => <SelectItem key={term.id} value={term.id}>{term.name}</SelectItem>)}</SelectContent></Select></div></>}</div>
           {view === "matrix" ? matrixQuery.isLoading ? <SubjectAssignmentTableSkeleton /> : matrixQuery.isError ? <div className="p-8 text-center text-sm text-muted-foreground">Unable to load the teaching matrix.</div> : matrixQuery.data ? <AssignmentMatrix matrix={matrixQuery.data} termId={termId} /> : null : isLoading ? (
