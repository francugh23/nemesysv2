...
async function createFixture(transaction: Prisma.TransactionClient) {
  ...
  await makeLegacyActiveCurriculumConfigurable(academicYear.id, transaction);
  const [user, term, jhsOffering] = await Promise.all([
    transaction.user.findFirstOrThrow({ where: { deletedAt: null }, select: { id: true } }),
    transaction.academicTerm.findFirstOrThrow({ where: { academicYearId: academicYear.id }, select: { id: true } }),
    transaction.subjectOffering.findFirstOrThrow({ where: { academicYearId: academicYear.id, gradeLevel: "7", deletedAt: null }, select: { id: true } }),
  ]);
  ...
  return { user, academicYear, term, offering, jhsOffering, academicCluster, techProCluster };
}
...
test("provisional SHS Offerings cannot materialize Student Subject Enrollments", async () => {
  await assert.rejects(prisma.$transaction(async (transaction) => {
    ...
    const enrollment = await transaction.enrollment.create({ data: { studentId: student.id, sectionId: section.id, academicYearId: fixture.academicYear.id, entryAcademicTermId: fixture.term.id, shsTrack: "ACADEMIC", createdById: fixture.user.id }, select: { id: true } });
