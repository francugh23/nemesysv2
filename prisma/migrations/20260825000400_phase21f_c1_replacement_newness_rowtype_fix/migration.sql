...
DECLARE
  ...
  replacement_xmin TEXT;
  replacement_xmin_status TEXT;
  exact_replacement_id_count INTEGER;
  same_enrollment_offering_replacement_term_membership_count INTEGER;
BEGIN
  ...
  END IF;
  SELECT participation.xmin::TEXT INTO replacement_xmin
  FROM "StudentSubjectEnrollment" participation
  WHERE participation."id" = NEW."replacementStudentSubjectEnrollmentId";
  ...
  replacement_xmin_status := pg_xact_status(replacement_xmin::xid8)::TEXT;
  IF replacement_xmin_status <> 'in progress' THEN
    SELECT COUNT(*) INTO exact_replacement_id_count
    FROM "StudentSubjectEnrollment"
    WHERE "id" = replacement_record."id";
    SELECT COUNT(*) INTO same_enrollment_offering_replacement_term_membership_count
    FROM "StudentSubjectEnrollment" participation
    JOIN "StudentSubjectEnrollmentTerm" membership
      ON membership."studentSubjectEnrollmentId" = participation."id"
    WHERE participation."enrollmentId" = replacement_record."enrollmentId"
      AND participation."subjectOfferingId" = replacement_record."subjectOfferingId"
      AND membership."academicTermId" = NEW."replacementAcademicTermId";
    RAISE EXCEPTION '%', format(
      'SHS Student Participation Correction replacement participation must be new: replacementStudentSubjectEnrollmentId=%s, sourceStudentSubjectEnrollmentId=%s, replacementSubjectOfferingId=%s, replacementEnrollmentId=%s, enrollmentStudentId=%s, enrollmentAcademicYearId=%s, xmin=%s, pg_xact_status=%s, txid_current_if_assigned=%s, pg_backend_pid=%s, nemesys.shs_student_participation_correction_id=%s, exactReplacementIdCount=%s, sameEnrollmentOfferingReplacementTermMembershipCount=%s',
      NEW."replacementStudentSubjectEnrollmentId",
      NEW."sourceStudentSubjectEnrollmentId",
      replacement_record."subjectOfferingId",
      replacement_record."enrollmentId",
      enrollment_record."studentId",
      enrollment_record."academicYearId",
      replacement_xmin,
      replacement_xmin_status,
      COALESCE(txid_current_if_assigned()::TEXT, 'NULL'),
      pg_backend_pid(),
      COALESCE(current_setting('nemesys.shs_student_participation_correction_id', true), 'NULL'),
      exact_replacement_id_count,
      same_enrollment_offering_replacement_term_membership_count
    );
  END IF;
  IF EXISTS (
